...
export function eseguiAcquisto() {
  ...
  if (qta <= 0) { showToast("Inserisci una quantità valida", 'warning'); return; }
  const oroDisp = oroDisponibile();
  if (oroDisp < totale) {
    showToast(`Oro insufficiente:\n  Servono: ${totale.toFixed(1).replace(/\.0$/, "")} mo\n  Disponibili: ${oroDisp.toFixed(1).replace(/\.0$/, "")} mo\n  Mancano: ${(totale - oroDisp).toFixed(1).replace(/\.0$/, "")} mo`, 'warning');
    return;
  }
  const cap = capacitaMagazzino();
  if (cap > 0) {
    ...
    if (valoreAggiunta > spazio) {
      showToast(`Capacità Magazzino insufficiente:\n  Spazio residuo: ${spazio.toFixed(1)} mo\n  Valore acquisto: ${valoreAggiunta.toFixed(1)} mo\n  Mancano: ${(valoreAggiunta - spazio).toFixed(1)} mo di spazio`, 'warning');
      return;
    }
  }
  ...
  document.getElementById("wh-inventory-body").addEventListener("change", e => {
    ...
    if (cap > 0) {
      ...
      if (delta > 0 && delta > spazioResiduoMagazzino()) {
        showToast(`Capacità Magazzino insufficiente:\n  Spazio residuo: ${spazioResiduoMagazzino().toFixed(1)} mo\n  Valore aggiunto: ${delta.toFixed(1)} mo\n  Mancano: ${(delta - spazioResiduoMagazzino()).toFixed(1)} mo di spazio`, 'warning');
        renderMagazzino();
        return;
      }
    }
