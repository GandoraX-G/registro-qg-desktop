...
export function validaStato(){
  ...
  if(!Array.isArray(state.speseExtra)) state.speseExtra = [];
  if(state.speseExtra.length > 0 && Array.isArray(state.registroAltriQG)){
    state.speseExtra.forEach(sp => {
      state.registroAltriQG.push({
        uid: sp.uid || uid(),
        nome: sp.descrizione || '',
        tipo: 'spesa',
        importo: sp.importo || 0,
        nota: '',
        saldato: true
      });
    });
